...
{
    internal class DbContext<T> where T : IModel
    {
        private DbContext()
        {
        ...
        {
            // Check to see if the object (inherited from Model) has a CollectionName attribute
        ...
        {
            // Check to see if the object (inherited from Model) has a ConnectionName attribute
        ...
        private static string GetConnectionNameFromType()
        {
            ...
            {
                if (typeof(Model).GetTypeInfo().IsAssignableFrom(modelType))
                {
                    // No attribute found, get the basetype
                    while (!modelType.GetTypeInfo().BaseType.Equals(typeof(Model)))
                    {
        ...
        {
            return config.GetSection("DbOptions").GetSection(GetConnectionName()).Value;
        }
